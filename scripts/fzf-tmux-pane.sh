...
if [[ "$TMUX_FZF_POPUP" == 0 ]]; then
  ...
  cat "$fifo2" &
  # MA TODO FIXME Seems to not work in newer version sof tmux, the -K option or the -R option
  #tmux popup -d '#{pane_current_path}' -xC -yC -w$TMUX_FZF_POPUP_WIDTH -h$TMUX_FZF_POPUP_HEIGHT -K -E -R "$envs bash $argsf"
  tmux popup -d '#{pane_current_path}' -xC -yC -w$TMUX_FZF_POPUP_WIDTH -h$TMUX_FZF_POPUP_HEIGHT -E "$envs bash $argsf"
fi
